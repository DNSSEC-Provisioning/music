...
func DdnsMgr(mconf *Config, done <-chan struct{}) {

	ddnsfetch := mconf.Internal.DdnsFetch
	ddnsupdate := mconf.Internal.DdnsUpdate
		...
		for {
			...
			case <-fetch_ticker.C:
				...
				for {
					...
					for {
						rl, hold, err = RLDdnsFetchRRset(fdop)
						if err != nil {
