...
func deSECmgr(conf *Config, done <-chan struct{}) {
	...
	var fetchlimit = viper.GetInt("signers.desec.limits.fetch") // per second
	var updatelimit = viper.GetInt("signers.desec.limits.update") // per second
...
//	}
	...
	// fetchsleep := 1.0 / float32(fetchlimit)
	// fetchsleep := 1.0 / float32(fetchlimit)
	// fetchdur := time.Duration(fetchsleep) * time.Minute
	// updatesleep := 1.0 / float64(updatelimit)
	// fmt.Printf("deSECmgr: fetchsleep: %f fetchdur: %v\n", fetchsleep, fetchdur.String())

	fetch_ticker := time.NewTicker(time.Minute)
	update_ticker := time.NewTicker(time.Minute)

	var fetchops, updateops int 
